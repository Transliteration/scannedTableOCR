...
def saveTableToXLSX(centers):
    ...
    # Save the file
    wb.save(ntpath.dirname(ntpath.normpath(args.image)) + '/' + '.'.join([str(elem) for elem in ntpath.basename(args.image).split(".")[0:-1]]) + ".xlsx")
...
def getTextAndConf(centers, crop_img):
    ...
    for i in range(len(centers)):
        ...
        if thval/(height*width) < 0.999 :
            data = pytesseract.image_to_string(ret, lang="rus+eng", config='--psm 6')
            text = data
...
parser = argparse.ArgumentParser(description='Convert image containing scanned table to xlsx file.')
parser.add_argument("-i", "--image", action='store', help="path to image to be processed", required=True)
...
if maxsize > 2200:
    img = resizeImage(img, max(int(220000/maxsize), 40))
